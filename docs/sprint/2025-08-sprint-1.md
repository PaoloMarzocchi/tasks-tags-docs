...
## Sprint Board (Area | Task | Stato)
| Area | Task | Stato |
|------|------|-------|
| Data | Migrazioni tags/tasks/task_tag | TODO |
| Data | Seeder base (utente demo, tag "Work", task vuoto) | TODO |
| BE | Endpoints /auth (login/logout/me) | TODO |
| BE | CRUD /tags (+validazioni) | TODO |
| BE | CRUD /tasks (+filtri q/status/tag + paginazione) | TODO |
| BE | Test feature (Pest/PHPUnit) | TODO |
| FE | /login | TODO |
| FE | Home: lista task + filtri + edit inline | TODO |
| FE | Hook API + MSW per sviluppare in parallelo | TODO |
| FE | Unit/component tests | TODO |
| QA | Smoke E2E su EPIC Gherkin | TODO |
| QA | Checklist DoD | TODO |
## Sprint Board (Kanban)

### ✅ FATTO
- Docs: Charter & Process (DoR/DoD)
- Docs: API Spec v1.0.0 (`docs/api.yaml`)
- Docs: Sprint backlog (Epic, US, Gherkin, board)
- Docs: ADR + Changelog
- Data: Migrazioni `tags`, `tasks`, `task_tag`
- Data: Seeder demo (tag "Work")
- BE: AuthController (stub login/logout/me)
- BE: CRUD Tag (index, store, show, update, destroy)
- BE: CRUD Task (index + filtri q/status/tag + paginazione, store, show, update, destroy)
- BE: Test feature Epic (Pest)
- DevOps: Env vars `.env.example` con `AUTH_DRIVER=bearer`

### ⏳ IN CORSO
- BE: Copertura test >70%
- BE: Auth reale (Sanctum/Passport)

### ❌ TODO
- Data: Seeder utenti demo
- FE: Setup progetto (React/Vite/Next)
- FE: /login (form + token bearer)
- FE: Home con lista task + filtri
- FE: Edit task (update status)
- FE: Hook API (da `docs/api.yaml`)
- FE: MSW (mock API)
- FE: Test unit/component
- QA: Test Plan (`/docs/qa/test-plan.md`)
- QA: Smoke E2E (EPIC Gherkin)
- QA: Checklist DoD
- DevOps: CI pipeline (lint + test BE/FE)
- DevOps: DB seed pipeline (stage)